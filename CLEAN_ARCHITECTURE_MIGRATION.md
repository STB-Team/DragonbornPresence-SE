...
3. Ассистент читает все затронутые файлы, проверяет зависимости и остаточные ссылки, затем выполняет целевую сборку и предусмотренный для блока smoke test.
...
- ветка: `dev`;
- опубликованный HEAD: `3dbb9bb refactor: inject game data source port`;
- `origin/dev` указывает на тот же commit;
- в worktree находится незавершённое извлечение `StbGameDataSource`;
...
- `CMakeLists.txt` содержит записи обоих незавершённых блоков, поэтому при commit необходимо выбирать hunks строго по активному блоку.

Текущий активный блок: **B5 — извлечь адаптер данных Skyrim True Believer**.
...
### B5. Извлечь `StbGameDataSource` — активный
...
- runtime smoke test показывает загрузку форм STB и прежние difficulty/deaths/stone/location/combat данные.

Планируемый commit: `refactor: extract Skyrim True Believer game adapter`.
...
- Release-сборка и Discord runtime smoke test успешны.

Планируемый commit: `refactor: extract Discord presence adapter`.
...
- Release-сборка и запуск без Discord/с Discord дают прежние диагностические результаты.
...
- Release-сборка и полный runtime smoke test успешны.

Планируемый commit: `refactor: separate application and STB runtime responsibilities`.
...
- формат Presence и state transitions покрываются наблюдаемыми application-тестами с fake ports там, где текущие тесты не защищают контракт;
- Release-сборка и application tests успешны.
...
- Release-сборка и runtime smoke test меню/loading/combat успешны.
...
- Release-сборка, упаковка ZIP и полный runtime smoke test успешны.

Планируемый commit: `refactor: reduce plugin module to composition root`.

## Проверка каждого блока

Минимальная автоматическая проверка перед commit:

```text
cmake --build --preset release
```

Если блок затрагивает core/application контракт или существующие тесты:
...
Runtime smoke test обязателен для блоков B5, B7, B8, B10, B12 и B13. Пользователь запускает сборку через текущее окружение MO2/Skyrim и сообщает наблюдаемый результат; ассистент сопоставляет его с критериями блока. Проверяются как минимум:

- plugin и Discord SDK инициализируются;
- лог содержит успешное разрешение STB forms;
- loading Presence сменяется игровым;
- difficulty, deaths, stone и location не изменились;
- combat Presence появляется и очищается;
- session start timestamp не сбрасывается при обновлениях;
- выход/ошибка Discord отключает только интеграцию, не Skyrim.
