...
[[nodiscard]] constexpr DiscordResultDetails DescribeResult(discord::Result result) noexcept
{
    ...
    switch (result) {
    case Ok: return {"Ok", "the operation completed successfully"};
...
public:
    ...
    {
        if (!IsActive() || !pendingActivitySignature_.empty()) return false;
        ...
        try {
            ...
            if (signature == lastActivitySignature_) return false;
...
private:
    ...
    void Disable(std::string_view reason) noexcept
    {
        ...
        pendingActivitySignature_.clear();
        pendingActivityCallbackTicks_ = 0;
        if (hadWork) LogFailure("safe shutdown", reason);
        core_.reset();
    }
...
public:
    ...
    void RegisterGameEventHandlers()
    {
        ...
        if (!discordClient_.Initialize(config_)) return;
...
private:
    ...
    {
        if (callbackThread_.joinable()) return;
